...
METRIC_RE = re.compile('GRID_METRIC val_macro_f1 ([0-9]*\\.?[0-9]+)')


def parse_list(raw: str, cast_fn):
    vals = []
    for token in re.split('[,\\s]+', raw.strip()):
        if token:
...
def build_run_id(lr, md, sub, col, lam):
    return f'lr{run_id_value(lr)}_md{run_id_value(md)}_sub{run_id_value(sub)}_col{run_id_value(col)}_lam{run_id_value(lam)}'
...
def run_one_config(args, lr, md, sub, col, lam):
    ...
    cmd = [sys.executable, args.script_path, '--data_path', args.data_path, '--indices_file', args.indices_file, '--seeds', args.seeds, '--early_stopping_rounds', str(args.early_stopping_rounds), '--n_estimators', str(args.n_estimators), '--learning_rate', str(lr), '--max_depth', str(md), '--min_child_weight', str(args.min_child_weight), '--gamma', str(args.gamma), '--subsample', str(sub), '--colsample_bytree', str(col), '--reg_lambda', str(lam), '--reg_alpha', str(args.reg_alpha), '--tree_method', args.tree_method, '--device', args.device, '--n_jobs', str(args.n_jobs)]
    if args.no_plots:
        cmd.append('--no_plots')
    ...
    row = {'run_id': run_id, 'learning_rate': lr, 'max_depth': md, 'subsample': sub, 'colsample_bytree': col, 'reg_lambda': lam, 'val_macro_f1': metric, 'return_code': proc.returncode, 'log_file': log_file}
    return row


def write_csv(path: str, rows: List[dict]):
    fieldnames = ['run_id', 'learning_rate', 'max_depth', 'subsample', 'colsample_bytree', 'reg_lambda', 'val_macro_f1', 'return_code', 'log_file']
...
def main():
    parser = argparse.ArgumentParser('Kaggle/Linux-safe grid search wrapper for xgboost_weighted.py')
    ...
    rows = []
    for i, (lr, md, sub, col, lam) in enumerate(itertools.product(lrs, mds, subs, cols, lams), start=1):
        run_id = build_run_id(lr, md, sub, col, lam)
    ...
    sorted_rows = sorted(rows, key=lambda r: (-1 if r['val_macro_f1'] is None else 0, -(r['val_macro_f1'] or -1.0)))
