...
def plot_block_confusion(block_cm: np.ndarray, group_size: int, n_classes: int, normalize: str, out_prefix: str):
    ...
    g = block_cm.shape[0]
    tick_labels = [f'{k * group_size}-{min((k + 1) * group_size - 1, n_classes - 1)}' for k in range(g)]
    ...
    ax.set_title(f'Aggregated Block Confusion Matrix (block={group_size}, normalize={normalize})')
    plt.tight_layout()
...
def train_one_seed(seed: int, train_dataset, val_dataset, test_dataset, input_dim: int, output_dim: int, lr: float, batch_size: int, epochs: int, patience: int, min_delta: float, min_epochs: int, device):
    set_seed(seed)
    g = torch.Generator()
    g.manual_seed(seed)
    ...
    model = NeuralNetwork(input_dim=input_dim, hidden_dims=[512, 256, 128, 64, 32], output_dim=output_dim, activation='relu').to(device)
    ...
    def run_eval(loader):
        ...
        all_probs = np.concatenate(all_probs) if all_probs else np.empty((0, output_dim), dtype=float)
        ...
        top3 = top_k_accuracy_score(all_true, all_probs, k=min(3, output_dim), labels=labels)
        top5 = top_k_accuracy_score(all_true, all_probs, k=min(5, output_dim), labels=labels)
    ...
    for epoch in range(epochs):
        ...
        for X, y in train_loader:
            ...
            loss.backward()
            optimizer.step()
        val_loss, val_acc, val_p, val_r, val_f1, val_top3, val_top5, _, _, _ = run_eval(val_loader)
    ...
    test_loss, test_acc, test_p, test_r, test_f1, test_top3, test_top5, cm, y_true, y_pred = run_eval(test_loader)
    metrics = {'loss': test_loss, 'acc': test_acc, 'macro_p': test_p, 'macro_r': test_r, 'macro_f1': test_f1, 'top3': test_top3, 'top5': test_top5}
...
def main():
    parser = argparse.ArgumentParser('10-seed training + aggregated confusion analysis (no model saving, no training plots)')
    ...
    parser.add_argument('--group_size', type=int, default=25, help='classes per block in aggregated confusion heatmap')
    ...
    parser.add_argument('--rep_classes', type=int, default=12, help='how many representative diseases to print')
    ...
    print(f'Seeds: {seeds}')
    print('=' * 70)
    train_dataset = diagnosticsDataset('filtered_diseases.csv', split='train', indices_file='split_indices.npz')
    val_dataset = diagnosticsDataset('filtered_diseases.csv', split='val', indices_file='split_indices.npz')
    test_dataset = diagnosticsDataset('filtered_diseases.csv', split='test', indices_file='split_indices.npz')
    ...
    all_pred = []
    for s in seeds:
        m, cm, y_true, y_pred = train_one_seed(seed=s, train_dataset=train_dataset, val_dataset=val_dataset, test_dataset=test_dataset, input_dim=input_dim, output_dim=output_dim, lr=args.lr, batch_size=args.batch_size, epochs=args.epochs, patience=args.patience, min_delta=args.min_delta, min_epochs=args.min_epochs, device=device)
        metrics_list.append(m)
        cms.append(cm)
        all_true.append(y_true)
        all_pred.append(y_pred)
        print(f"[seed {s}] loss={m['loss']:.4f} | acc={m['acc']:.4f} | macroP/R/F1={m['macro_p']:.4f}/{m['macro_r']:.4f}/{m['macro_f1']:.4f} | top3={m['top3']:.4f} | top5={m['top5']:.4f}")
    ...
    plot_block_confusion(block_cm, args.group_size, output_dim, args.normalize, out_prefix)
    ...
    for rank, idx in enumerate(idxs, start=1):
        ...
        if c <= 0:
            continue
        print(f'{rank:2d}. {label_names[i]}  →  {label_names[j]}   (count={c})')
    y_true_all = np.concatenate(all_true)
    y_pred_all = np.concatenate(all_pred)
    per_p, per_r, per_f1, per_support = precision_recall_fscore_support(y_true_all, y_pred_all, labels=np.arange(output_dim), zero_division=0)
    valid = per_support > 0
    ...
    for i in reps:
        print(f'{label_names[i]} | {int(per_support[i])} | {per_p[i]:.4f} | {per_r[i]:.4f} | {per_f1[i]:.4f}')
