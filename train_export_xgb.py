...
def eval_metrics(y_true: np.ndarray, y_prob: np.ndarray, num_classes: int):
    ...
    return {'acc': acc, 'macro_p': macro_p, 'macro_r': macro_r, 'macro_f1': macro_f1, 'top3': top3, 'top5': top5}


def train_one_seed(seed: int, X_train, y_train, X_val, y_val, X_test, y_test, class_weights: np.ndarray, params: dict, esr: int, num_classes: int) -> Tuple[Dict[str, Any], xgb.Booster]:
    set_seed(seed)
    ...
    metrics = {'seed': int(seed), 'best_val_macro_f1': float(val_m['macro_f1']), 'test_acc': float(test_m['acc']), 'test_macro_p': float(test_m['macro_p']), 'test_macro_r': float(test_m['macro_r']), 'test_macro_f1': float(test_m['macro_f1']), 'test_top3': float(test_m['top3']), 'test_top5': float(test_m['top5'])}
    return (metrics, bst)
...
def main():
    parser = argparse.ArgumentParser('Train class-weighted XGBoost + export backend bundle')
    ...
    parser.add_argument('--device', type=str, default='auto', help='XGBoost device: auto | cpu | cuda')
    ...
    best_val_f1 = -1.0
    for s in seeds:
        metrics, bst = train_one_seed(seed=s, X_train=X_train, y_train=y_train, X_val=X_val, y_val=y_val, X_test=X_test, y_test=y_test, class_weights=class_w, params=params, early_stopping_rounds=args.early_stopping_rounds, num_classes=num_classes)
        all_metrics.append(metrics)
        print(f"[seed {s}] bestValF1={metrics['best_val_macro_f1']:.4f} | testF1={metrics['test_macro_f1']:.4f} | acc={metrics['test_acc']:.4f} | top3={metrics['test_top3']:.4f} | top5={metrics['test_top5']:.4f}")
    ...
    cfg = XGBBundleConfig(bundle_format='xgb_classifier_bundle_v1', data_path=args.data_path, data_sha256=sha256_file(args.data_path), indices_file=args.indices_file, indices_sha256=sha256_file(args.indices_file), label_column=args.label_column, feature_cols=feature_cols, num_classes=num_classes, n_estimators=args.n_estimators, learning_rate=args.learning_rate, max_depth=args.max_depth, min_child_weight=args.min_child_weight, gamma=args.gamma, subsample=args.subsample, colsample_bytree=args.colsample_bytree, reg_lambda=args.reg_lambda, reg_alpha=args.reg_alpha, tree_method=args.tree_method, early_stopping_rounds=args.early_stopping_rounds, seeds=seeds, device=resolved_device)
    ensure_dir(args.export_dir)
    booster_raw = best_bst.save_raw()
    bundle = {'format': cfg.bundle_format, 'config': asdict(cfg), 'feature_cols': feature_cols, 'label_column': args.label_column, 'label_names': label_names, 'xgb_booster_raw': booster_raw, 'best_seed_metrics': best_metrics, 'all_seed_metrics': all_metrics, 'summary_mean_std': summary}
