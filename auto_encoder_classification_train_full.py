...
def resolve_ae_checkpoint(requested_path: str) -> str:
    ...
    if not raw or raw.lower() == "auto":
        ...
        raise FileNotFoundError(
            "Could not auto-resolve an autoencoder checkpoint. "
            f"Looked for '{AE_CKPT_GLOB_DEFAULT}' and fallback '{AE_CKPT_LEGACY_FALLBACK}'. "
            "Pass --ae_checkpoint with an exact .pth path or a glob pattern."
        )
    ...
    if any(ch in raw for ch in "*?[]"):
        ...
        if candidates:
            return candidates[0]
        raise FileNotFoundError(
            f"No checkpoint matched the pattern: {raw}"
        )
    ...
    raise FileNotFoundError(
        f"Autoencoder checkpoint not found: {raw}"
    )


class EncoderClassifier(nn.Module):
    """Pretrained encoder + classification head, trained end-to-end."""
...
def train_one_seed(
    ...
    z_dim = ae.encoder[-1].out_features  # latent dim from encoder's last Linear
    ...
    # ---- Separate LRs: lower for pretrained encoder, higher for new head ----
...
def main():
    # Keep output robust on Windows consoles/log redirection.
    try:
    ...
    parser = argparse.ArgumentParser(
        "AE end-to-end fine-tuned classifier on FULL unbalanced dataset — 10 seeds + aggregated confusion")
    parser.add_argument("--data_path", type=str,
                        default="diseases.csv")
    ...
    parser.add_argument("--encoder_lr", type=float, default=1e-4,
                        help="Learning rate for the pretrained encoder (lower to avoid destroying pretrained features)")
    ...
    parser.add_argument("--latent_dim", type=int, default=64,
                        help="Latent dim of the autoencoder (must match checkpoint)")
    parser.add_argument("--ae_checkpoint", type=str, default=AE_CKPT_PATH_DEFAULT,
                        help="Path or glob for trained autoencoder checkpoint; use 'auto' to pick the tuned latent-64 checkpoint")
    ...
    # ---- Load datasets (FULL unbalanced split) ----
    ...
    # ---- Load pretrained autoencoder (encoder will be fine-tuned) ----
    resolved_ae_checkpoint = resolve_ae_checkpoint(args.ae_checkpoint)
    print("Loading pretrained autoencoder for end-to-end fine-tuning...")
    print(f"Autoencoder checkpoint: {resolved_ae_checkpoint}")

    ae = Autoencoder(input_dim=input_dim, latent_dim=args.latent_dim).to(device)
    ckpt = torch.load(resolved_ae_checkpoint, map_location=device)
