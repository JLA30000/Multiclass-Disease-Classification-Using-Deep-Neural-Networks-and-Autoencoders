...
def resolve_ae_checkpoint(requested_path: str) -> str:
    ...
    if not raw or raw.lower() == 'auto':
        ...
        raise FileNotFoundError(f"Could not auto-resolve an autoencoder checkpoint. Looked for '{AE_CKPT_GLOB_DEFAULT}' and fallback '{AE_CKPT_LEGACY_FALLBACK}'. Pass --ae_checkpoint with an exact .pth path or a glob pattern.")
    if any((ch in raw for ch in '*?[]')):
        candidates = sorted(glob.glob(raw))
        if candidates:
            return candidates[0]
        raise FileNotFoundError(f'No checkpoint matched the pattern: {raw}')
    if os.path.isfile(raw):
        return raw
    raise FileNotFoundError(f'Autoencoder checkpoint not found: {raw}')
...
def compute_class_weights_from_train(train_labels, num_classes: int):
...
def train_one_seed(seed: int, ae: nn.Module, train_ds, val_ds, test_ds, batch_size: int, lr: float, encoder_lr: float, epochs: int, patience: int, min_delta: float, min_epochs: int, hidden_dims, weights_tensor: torch.Tensor, device: torch.device):
    set_seed(seed)
...
def main():
    try:
        ...
        pass
    parser = argparse.ArgumentParser('AE end-to-end fine-tuned classifier on FULL unbalanced dataset — 10 seeds + aggregated confusion')
    ...
    parser.add_argument('--lr', type=float, default=0.0001, help='Learning rate for the classifier head')
    parser.add_argument('--encoder_lr', type=float, default=0.0001, help='Learning rate for the pretrained encoder (lower to avoid destroying pretrained features)')
    ...
    parser.add_argument('--latent_dim', type=int, default=64, help='Latent dim of the autoencoder (must match checkpoint)')
    parser.add_argument('--ae_checkpoint', type=str, default=AE_CKPT_PATH_DEFAULT, help="Path or glob for trained autoencoder checkpoint; use 'auto' to pick the tuned latent-64 checkpoint")
    ...
    print('=' * 70)
    rac = resolve_ae_checkpoint(args.ae_checkpoint)
    print('Loading pretrained autoencoder for end-to-end fine-tuning...')
    print(f'Autoencoder checkpoint: {rac}')
    ae = Autoencoder(input_dim=input_dim, latent_dim=args.latent_dim).to(device)
    ckpt = torch.load(rac, map_location=device)
    state = ckpt['model_state_dict'] if isinstance(ckpt, dict) and 'model_state_dict' in ckpt else ckpt
    ae.load_state_dict(state)
    print(f'Autoencoder loaded (latent_dim={args.latent_dim})')
    print(f'Encoder LR: {args.encoder_lr}  |  Classifier head LR: {args.lr}')
    ...
    all_pred = []
    for s in seeds:
        val_m, test_m, cm, y_true, y_pred = train_one_seed(seed=s, ae=ae, train_ds=train_ds, val_ds=val_ds, test_ds=test_ds, batch_size=args.batch_size, lr=args.lr, encoder_lr=args.encoder_lr, epochs=args.epochs, patience=args.patience, min_delta=args.min_delta, min_epochs=args.min_epochs, hidden_dims=args.hidden_dims, weights_tensor=weights_tensor, device=device)
        vml.append(val_m)
        tml.append(test_m)
        cms.append(cm)
    ...
    if args.out_prefix is None:
        out_prefix = f'AEclf_full_block_confusion_g{args.group_size}_seeds{seeds[0]}-{seeds[-1]}'
    else:
