...
def train_one_seed(seed: int, train_ds, val_ds, test_ds, batch_size: int, lr: float, epochs: int, patience: int, min_delta: float, min_epochs: int, hidden_dims, weights_tensor: torch.Tensor, device: torch.device):
    set_seed(seed)
    ...
    model = NeuralNetwork(input_dim=input_dim, hidden_dims=hidden_dims, output_dim=num_classes, activation='relu').to(device)
...
def main():
    parser = argparse.ArgumentParser('Class-weighted CE classifier — 10 seeds + aggregated confusion (auto-run, no manual steps)')
