...
set "RC1=%OUTDIR%\results.csv"
set "BEST_TXT=%OUTDIR%\best_run.txt"

set "METRIC_KEY=best_val_macro_f1_mean"

echo run_id,lr,batch_size,weight_decay,patience,metric,log_file> "%RC1%"
        ...
        if errorlevel 1 (
          ...
          if not defined METRIC (
            ...
            if not defined BM (
              ...
              for /f %%C in ('python -c "import sys;print(int(float(sys.argv[1]).__gt__(float(sys.argv[2]))))" !METRIC! !BM!') do (
                if "%%C"=="1" (
                  set "BM=!METRIC!"
                  set "BRI=!RUN_ID!"
                  set "BEST_LR=%%L"
