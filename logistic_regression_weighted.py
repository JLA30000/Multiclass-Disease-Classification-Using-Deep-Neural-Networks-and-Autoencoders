...
def train_one_seed(seed: int, train_ds, val_ds, test_ds, batch_size: int, lr: float, epochs: int, patience: int, min_delta: float, min_epochs: int, weight_decay: float, weights_tensor: torch.Tensor, device: torch.device):
    import copy
...
def main():
    ...
    parser = argparse.ArgumentParser('Class-weighted Logistic Regression (softmax) - multi-seed + aggregated confusion')
    ...
    parser.add_argument('--weight_decay', type=float, default=1e-06, help='L2 regularization (Adam weight_decay).')
    ...
    all_pred = []
    for s in seeds:
        val_m, test_m, cm, y_true, y_pred = train_one_seed(seed=s, train_ds=train_ds, val_ds=val_ds, test_ds=test_ds, batch_size=args.batch_size, lr=args.lr, epochs=args.epochs, patience=args.patience, min_delta=args.min_delta, min_epochs=args.min_epochs, weight_decay=args.weight_decay, weights_tensor=weights_tensor, device=device)
        vml.append(val_m)
        tml.append(test_m)
        cms.append(cm)
    ...
    test_f1_mu, test_f1_sd = mean_std([m['macro_f1'] for m in tml])
    ...
    for ext in ['.png', '.jpg']:
        ...
        if os.path.exists(fpath):
            ...
            print(f'WARNING: {fpath} was NOT saved!')
