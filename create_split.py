...
idx = np.arange(len(y))
idx_train_pool, idx_test = train_test_split(idx, test_size=TEST_SIZE, stratify=y, random_state=SEED)
y_train_pool = y[idx_train_pool]
vfotp = VAL_SIZE / (1.0 - TEST_SIZE)
idx_train, idx_val = train_test_split(idx_train_pool, test_size=vfotp, stratify=y_train_pool, random_state=SEED)
...
np.savez(SAVE_FILE, train_idx=idx_train, val_idx=idx_val, test_idx=idx_test, seed=SEED, test_size=TEST_SIZE, val_size=VAL_SIZE, label_col=LABEL_COL)
